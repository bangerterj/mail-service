...
```jsonc
{
  "key_live_meal_a1b2c3...": {
    ...
    "configurationSet": "meal-picker",                 // optional; SES config set name
...
If the app config carries a `configurationSet`, pass it through as SES's
`ConfigurationSetName`; omit the field entirely when absent. One config set per app gives
per-app delivery/bounce metrics and per-app CloudWatch alarms for free — it is the
low-cost substitute for SES Tenants. The service must work correctly when it is absent.
...
3. For each domain: run `verify-domain`, add the printed DNS records, wait, then
   `check-domain` until verified. The MAIL FROM subdomain is always `mail.<domain>`, and
   "Behavior on MX failure" stays at **Use default MAIL FROM domain**. The script prints
   records only — it must never write DNS, since these domains carry live traffic and
   live email from other providers.
...
## 14a. Already provisioned

See `AWS_HANDOFF.md` for confirmed environment facts: region, credential env var names,
the IAM policy's exact allowed actions, verified domain status, sandbox limits, mailbox
simulator addresses for tests, and the account settings that were deliberately turned off
(engagement tracking, auto validation, dedicated IPs). Read it before writing the SES
provider or the domain scripts.
